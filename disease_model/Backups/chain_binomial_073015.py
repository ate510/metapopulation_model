...
###################################################
def update_SI(met_id, d_Susc, d_Infc, num_child_zeros, num_adult_zeros, d_metro_infected_child, d_infected_adult, time_step):
#num_child_zeros and num_adult_zeros are per metro area
    
    #children
    d_Susc[(met_id, 'C')] -= num_child_zeros
    d_Infc[(met_id, 'C')] += num_child_zeros
    num_infc_child = d_Infc[(met_id, 'C')]
    
    #adults
    d_Susc[(met_id, 'A')] -= num_adult_zeros)
    d_Infc[(met_id, 'A')] += num_adult_zeros)
...
#  and return the total number of infected individuals (num_infected)
    ...
    num_infected = ((d_infected_child[time_step]) + (d_infected_adult[time_step])) 
    ...
    #new_recov_adult = 0
        ...
        for met_id in metro_ids:
            ...
            prob = susc_infc_binomial_prob_child(met_id, d_Infc, C, beta)
            new_cases_child = np.random.binomial(Susc_C, prob)
...
#            #child
#            d_Susc[(met_id, 'C')] -= new_cases_child
#            d_Infc[(met_id, 'C')] += new_cases_child
#            num_infc_child = d_Infc[(met_id, 'C')]
#            d_metro_infected_child[(met_id, time_step)] = num_infc_child
#            
#            #adults
#            d_Susc[(met_id, 'A')] -= new_cases_adult)
#            d_Infc[(met_id, 'A')] += new_cases_adult)
#            num_infc_adult = d_Infc[(met_id, 'A')]
#            d_metro_infected_adult[(met_id, time_step)] = num_infc_adult
        ...
        # subtract from Ii, add to Ri
            ...
            #num_recov_child = d_Recv[(met_id, 'C')]
            
            #adults
            d_Infc[(met_id, 'A')] -= new_recov_adult)
            d_Recv[(met_id, 'A')] += new_recov_adult)
            #num_recov_adult = d_Recv[(met_id, 'A')]
        ...
        d_newly_infected_child[time_step] = num_infc_child
        d_newly_infected_child[time_step] = num_infc_adult
