...
def susc_infc_loop_adult (met_id, d_Infc, new_cases, C):
    ...
    if susc_infc_event_adult(infected_degree_of_s_A, beta):
        new_cases = new_cases + 1  
...
###################################################
def update_SI(metro_zeros, d_Susc, d_Infc, num_child_zeros, num_adult_zeros, num_infc_child, num_infc_adult, d_infected_child, d_infected_adult, time_step):
#num_child_zeros and num_adult_zeros are per metro area
    ...
    d_infected_child[time_step] = num_infc_child
    d_infected_adult[time_step] = num_infc_adult
...
###################################################
def chain_binomial_one_simulation(d_metro_age_pop, metro_ids, beta, gamma, air_network, contact_network, num_metro_zeros, num_child_zeros, num_adult_zeros, total_pop, d_age, C):
...
#  and return the total number of infected individuals (num_infected)
    ...
    d_newly_infected_child, d_newly_infected_adult = {}, {} # this dictionary will keep track of how many infected in current time step
    ...
    num_infc_child = 0
    num_infc_adult = 0
    update_SI(metro_zeros, d_Susc, d_Infc, num_child_zeros, num_adult_zeros, num_infc_child, num_infc_adult, d_newly_infected_child, d_newly_infected_adult, time_step)
    ...
    for met_id in metro_ids:
        # child
        Susc_C = d_Susc[(met_id, 'C')]
        for s in range(0, Susc_C):
        #while Susc_C:
            susc_infc_loop_child(met_id, d_Infc, new_cases_child, C)            
        #adult
        Susc_A = d_Susc[(met_id, 'A')]
        for s in range(0, Susc_A):
            susc_infc_loop_adult(met_id, d_Infc, new_cases_adult, C)
    ...
    #subtract from Si, add to Ii
        ...
        d_Susc[(met_id, 'C')] = ((d_Susc[(met_id, 'C')]) - new_cases_child)
        d_Infc[(met_id, 'C')] = ((d_Infc[(met_id, 'C')]) + new_cases_child)
        num_infc_child = (num_infc_child + (d_Infc[(met_id, 'C')]))
        #adults
        d_Susc[(met_id, 'A')] = ((d_Susc[(met_id, 'A')]) - new_cases_adult)
        d_Infc[(met_id, 'A')] = ((d_Infc[(met_id, 'A')]) + new_cases_adult)
        num_infc_adult = (num_infc_adult + (d_Infc[(met_id, 'A')]))
        
        d_infected_child[time_step] = num_infc_child
        d_infected_adult[time_step] = num_infc_adult
        
    #update_SI(metro_ids, d_Susc, d_Infc, new_cases_child, new_cases_adult, num_infc_child, num_infc_adult, d_newly_infected_child, d_newly_infected_adult, time_step)
