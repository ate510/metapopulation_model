...
# returns infected degree based on contact probabilities for adults
    ...
    return ((C_cc*infected_degree_child)+(C_ca*infected_degree_adult))
...
###################################################
def chain_binomial_one_simulation(beta, gamma, contact_network, num_patient_zeros, total_pop, d_age):
...
#  and return the total number of infected individuals (num_infected)
    ...
    Susc_A = [i for i in Susc if d_age[i] == "adult"]
    #for i in Susc:
    #    if d_age[i] == "adult":
    #        Susc_A.append(i)
    Susc_C = [i for i in Susc if d_age[i] == "child"]
    #for i in Susc:
    #    if d_age[i] == "child":
    #        Susc_C.append(i)
    Infc = [] # List of infecteds/infectious (empty for now)
    Infc_A = [] # List of infected adults
    Infc_C = [] # List of infected children
