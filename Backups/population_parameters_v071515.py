...
###################################################
def aggregate_contacts (ch_1, ch_2, ad_1, ad_2):
# using Germany data
    ...
    dict_contacts, germ_contact_ages = import_contact_matrix(germ_contact_data, 0, 1, 2)
    ...
    child_ages, adult_ages = sort_contact_keys_by_age(dict_contacts, germ_contact_ages)
    ...
    dict_germ_pop = import_germany_pop_data(germ_pop_data, 4, 8)
    ...
    for int_age in child_first_key:
        age = contact_key_dict[int_age] #grab age formatted for contact dict
        # multiply contacts by pop of contact age group
        contacts = sum(([dict_contacts[age, (pop_key_dict[contact_age])]) * (dict_germ_pop[(pop_key_dict[contact_age])]) for contact_age in adult_ages])
    ...
    return children, adults#dict_agg_contacts
...
def aggregate_participants ():
    ...
    dict_agg_contacts = aggregate_contacts(ch_1, ch_2, ad_1, ad_2)
...
if __name__ == "__main__":
    ...
    child, adults = aggregate_contacts(ch_1, ch_2, ad_1, ad_2)
